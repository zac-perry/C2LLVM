 ...
 */
struct id_entry *install(const char *name, int blev)
{
 ...
 */
struct id_entry *lookup(const char *name, int blev)
{
   struct id_entry *p;
...
char *slookup(const char *str)
{
   ...
   for (p = str_table[k]; p; p = p->s_next)
      if ((p->s_ptr) && (strcmp(str, p->s_ptr) == 0))
         return (p->s_ptr);
